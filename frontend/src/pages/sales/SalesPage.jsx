...
function NewSOForm({ onClose }) {
  ...
  const mut = useMutation({
    ...
    onSuccess: () => { qc.invalidateQueries(['sales']); onClose(); toast.success('Sales order created') },
  ...
  return (
    <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50 p-4">
      <div className="bg-white rounded-2xl shadow-xl w-full max-w-2xl max-h-[90vh] overflow-y-auto p-6">
        <h3 className="text-lg font-bold mb-4">New Sales Order</h3>
            ...
            <div key={i} className="grid grid-cols-12 gap-2 mb-2 items-end">
              <div className="col-span-5">
                <select className="input text-sm" value={line.product_id} onChange={e => pickProduct(i, e.target.value)}>
                  ...
                  {products?.map(p => <option key={p.id} value={p.id}>{p.name} (Free: {Number(p.free_to_use_qty).toFixed(0)})</option>)}
                </select>
              </div>
              ...
              <div className="col-span-3"><input className="input text-sm" type="number" placeholder="Price" value={line.unit_price} onChange={e => setLine(i, 'unit_price', e.target.value)} /></div>
              <div className="col-span-1"><button className="text-red-500 hover:text-red-700 text-lg" onClick={() => removeLine(i)}>×</button></div>
            </div>
          ))}
        </div>
...
export default function SalesPage() {
  ...
  const confirmMut = useMutation({ mutationFn: id => api.post(`/sales/${id}/confirm`), onSuccess: () => { qc.invalidateQueries(['sales']); toast.success('Order confirmed') }, onError: e => toast.error(e.response?.data?.detail || 'Error') })
  const cancelMut = useMutation({ mutationFn: id => api.post(`/sales/${id}/cancel`), onSuccess: () => { qc.invalidateQueries(['sales']); toast.success('Order cancelled') }, onError: e => toast.error(e.response?.data?.detail || 'Error') })

  return (
    <div>
      <div className="flex items-center justify-between mb-6">
        <h2 className="text-2xl font-bold text-gray-900">Sales Orders</h2>
        <button className="btn-primary flex items-center gap-2" onClick={() => setShowNew(true)}><Plus size={18} /> New Sales Order</button>
      </div>
      {isLoading ? <div className="text-gray-500">Loading…</div> : (
        <div className="overflow-x-auto">
          <table className="w-full bg-white rounded-xl shadow-sm border border-gray-200">
            <thead className="bg-gray-50">
              <tr>{['Order', 'Customer', 'Date', 'Status', 'Lines', 'Actions'].map(h => <th key={h} className="px-4 py-3 text-left text-xs font-semibold text-gray-500 uppercase">{h}</th>)}</tr>
            </thead>
            <tbody className="divide-y divide-gray-100">
              {orders?.map(o => (
                <tr key={o.id} className="hover:bg-gray-50">
                  <td className="px-4 py-3 font-mono font-semibold text-blue-600">{o.name}</td>
                  ...
                  <td className="px-4 py-3">
                    <div className="flex items-center gap-2">
                      <Link to={`/sales/${o.id}`} className="p-1 text-blue-600 hover:text-blue-700" title="View"><Eye size={16} /></Link>
                      {o.status === 'draft' && <button className="p-1 text-green-600 hover:text-green-700" title="Confirm" onClick={() => confirmMut.mutate(o.id)}><CheckCircle size={16} /></button>}
                      {['draft', 'confirmed'].includes(o.status) && <button className="p-1 text-red-500 hover:text-red-700" title="Cancel" onClick={() => cancelMut.mutate(o.id)}><XCircle size={16} /></button>}
                    </div>
                  </td>
                </tr>
              ))}
              {!orders?.length && <tr><td colSpan={6} className="text-center py-8 text-gray-400">No sales orders yet</td></tr>}
            </tbody>
          </table>
        </div>
      )}
